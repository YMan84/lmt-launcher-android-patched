    ...
    public MediaControllerCompat(Context context, MediaSessionCompat mediaSessionCompat) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= 24) {
                mediaControllerImplBase = new MediaControllerImplApi24(context, sessionToken);
            } else if (Build.VERSION.SDK_INT >= 23) {
                mediaControllerImplBase = new MediaControllerImplApi23(context, sessionToken);
            } else if (Build.VERSION.SDK_INT >= 21) {
                mediaControllerImplBase = new MediaControllerImplApi21(context, sessionToken);
            } else {
                mediaControllerImplBase = new MediaControllerImplBase(sessionToken);
            }
        } catch (RemoteException e) {
            Log.w(TAG, "Failed to create MediaControllerImpl.", e);
            mediaControllerImplBase = null;
        }
