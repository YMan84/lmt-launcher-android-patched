...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (getChildCount() > 0) {
            if (this.mShouldReverseLayout ^ this.mStackFromEnd) {
                ...
                iFixLayoutEndGap = fixLayoutStartGap(i4, recycler, state, DEBUG);
            } else {
                int iFixLayoutStartGap = fixLayoutStartGap(i3, recycler, state, true);
                i4 = i3 + iFixLayoutStartGap;
                i5 = i2 + iFixLayoutStartGap;
                iFixLayoutEndGap = fixLayoutEndGap(i5, recycler, state, DEBUG);
            }
    ...
    int scrollBy(int i, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int iFill = this.mLayoutState.mScrollingOffset + fill(recycler, this.mLayoutState, state, DEBUG);
        if (iFill < 0) {
            return 0;
        }
        if (iAbs > iFill) {
    ...
    public View onFocusSearchFailed(View view, int i, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        ensureLayoutState();
        ensureLayoutState();
        updateLayoutState(iConvertFocusDirectionToLayoutDirection, (int) (this.mOrientationHelper.getTotalSpace() * MAX_SCROLL_FACTOR), DEBUG, state);
        this.mLayoutState.mScrollingOffset = Integer.MIN_VALUE;
        this.mLayoutState.mRecycle = DEBUG;
    ...
    void validateChildOrder() {
        ...
        if (this.mShouldReverseLayout) {
            for (int i = 1; i < getChildCount(); i++) {
                ...
                if (position2 < position) {
                    ...
                    sb.append(decoratedStart2 >= decoratedStart ? DEBUG : true);
        ...
        for (int i2 = 1; i2 < getChildCount(); i2++) {
            ...
            if (position3 < position) {
                ...
                sb2.append(decoratedStart3 >= decoratedStart ? DEBUG : true);
